  ...
  const fetchMyGigs = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('jobs_services')
        .select('id, user_id, title, description, category, price, delivery_days, photo_urls, status, applications_count, average_rating, review_count, packages, created_at, updated_at')
        .eq('user_id', user.id)
