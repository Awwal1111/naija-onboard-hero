...
-- Foreign key constraint for wallet_transactions
ALTER TABLE public.wallet_transactions 
ADD CONSTRAINT fk_safepay_id 
FOREIGN KEY (safepay_id) REFERENCES public.safepay_transactions(id) ON DELETE CASCADE;

-- Create database functions for SafePay operations
...
BEGIN
  ...
  -- Record transaction
  INSERT INTO public.wallet_transactions (user_id, safepay_id, type, amount)
  VALUES (v_buyer_id, p_safepay_id, 'escrow_hold', -v_amount);
...
BEGIN
  ...
  -- Remove from buyer's escrow and add to seller's balance
  UPDATE public.user_wallets 
  SET escrow_hold = escrow_hold - v_amount,
      updated_at = NOW()
  WHERE user_id = v_buyer_id;

  -- Add to seller's balance (create wallet if not exists)
  ...
  -- Record transactions
  INSERT INTO public.wallet_transactions (user_id, safepay_id, type, amount)
  VALUES (v_buyer_id, p_safepay_id, 'escrow_release_to_seller', 0);

  INSERT INTO public.wallet_transactions (user_id, safepay_id, type, amount)
  VALUES (v_seller_id, p_safepay_id, 'escrow_release_from_buyer', v_amount);
...
BEGIN
  ...
  IF v_status = 'active' THEN
    ...
    INSERT INTO public.wallet_transactions (user_id, safepay_id, type, amount)
    VALUES (v_buyer_id, p_safepay_id, 'escrow_refund_to_buyer', v_amount);
  END IF;
...
CREATE TRIGGER update_safepay_timestamp
