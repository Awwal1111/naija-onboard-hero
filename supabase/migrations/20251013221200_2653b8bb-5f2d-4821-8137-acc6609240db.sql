...
-- Add missing timestamp columns for SafePay state tracking
...
-- Remove duplicate column (keeping cancel_requester_id)
ALTER TABLE public.safepay_transactions
DROP COLUMN IF EXISTS cancellation_requester_id;

-- Function: Mark SafePay work as complete
...
-- Function: Request SafePay cancellation
...
-- Function: Approve SafePay cancellation
...
-- Function: File SafePay dispute
CREATE OR REPLACE FUNCTION public.file_dispute_safepay(
  p_safepay_id uuid,
  p_reason text
)
RETURNS void
...
-- Create auto-release cron job (runs every hour)
-- This will auto-release funds when auto_release_at time is reached
