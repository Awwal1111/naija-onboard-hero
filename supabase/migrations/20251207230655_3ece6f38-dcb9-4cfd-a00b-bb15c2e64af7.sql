-- Fix: Remove non-existent 'skills' column reference from get_personalized_feed function
...
BEGIN
  ...
  user_profile AS (
    -- Phase 1: Get current user's profile for matching (removed skills)
    SELECT 
      pr.state_name, 
      pr.lga_name, 
      pr.area,
